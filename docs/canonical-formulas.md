...
```
revenue   := SUM(retail_amount) WHERE operation_kind = :sale   ; gross basis
net_sales := SUM(net_sales)     WHERE operation_kind = :sale   ; YM only; nil-safe (WB/Ozon → 0)
```
- **Бизнес-смысл**: `revenue` — сумма в рознице по gross-базису (до комиссий МП, до возвратов). Для YM `retail_amount` = `(BUYER + subsidy) × qty` (gross), см. §6.3.
- **`net_sales`** (spec 012): post-discount сумма (`BUYER × qty` для YM). `nil` для WB/Ozon (там нет платформенного дисконта, net == gross), поэтому агрегат `by-article`/`by-sku` берёт `(or (:net-sales %) 0)` → для WB/Ozon `net_sales = 0.0`, а их `revenue` не меняется. Базис поля: **net** (у `revenue` — **gross**).
- **Единицы**: руб.
...
- **RFC-15 YM-исключение (spec 012, 2026-07-01)**: на YM **sale**-строке `for_pay` **может быть `< 0`** (убыточный SKU: `BUYER − commissions < 0`). `Math/abs` снят ТОЛЬКО на YM sale-ветке; return сохраняет `+abs`. Агрегат `mp_payout = Σsale − Σreturn` корректно складывает отрицательные sale (P7). WB/Ozon — без изменений (`≥ 0`).
- **Единицы**: руб.
...
|---|---|---|---|
| `retail_price` | decimal | руб/шт | розничная цена (gross). YM: `BUYER + subsidy` per-unit (§6.3) |
| `retail_amount` | decimal | руб | `retail_price × quantity` (**gross-базис**); **всегда положительное**. YM: `(BUYER + subsidy) × qty` |
| `net_sales` | decimal / nil | руб | **net-базис** (post-discount). YM: `BUYER × qty`; **`nil` для WB/Ozon** (net == gross). Spec 012 |
...
|---|---|---|---|
| `for_pay` | decimal | **≥ 0** (WB/Ozon); **YM sale допускает `< 0`** (spec 012) | net payout от МП; для return — абсолютное значение (+abs) того, что МП вернёт себе. YM: `BUYER − Σ(non-ad commissions)`; убыточный SKU → sale `for_pay < 0` (RFC-15 YM-исключение, §6.3). Инвариант: `for_pay ≤ retail_amount` |
...
**Три ценовых базиса YM (spec 012, CORRECTED 2026-07-01, каждое money-поле указывает свой базис — P6):**

| Базис | Формула | Семантика | Canonical field |
|---|---|---|---|
| **gross** | `(BUYER + subsidy) × qty` | pre-discount учётная цена (== TS `realisation`) | `retail_amount`, `retail_price` (per-unit) |
| **net_sales** | `BUYER × qty` | цена, которую заплатил покупатель после промо (== TS `sales`) | `net_sales` (**НОВОЕ**) |
| **payout** | `BUYER − Σ(non-ad commissions)` | к выплате продавцу (== TS `toTransfer`) | `for_pay` |

> ⚠️ **Поле `MARKETPLACE` из stats/orders — это НЕ gross, а прокси размера субсидии** (== SUBSIDY/ACCRUAL на 96% позиций, < BUYER на 91%). Ground-truth сверка (апрель + TrueStats anchor): `BUYER == TS sales` (net), `BUYER + subsidy == TS realisation` (gross). Старая формула (`gross = MARKETPLACE`, `retail_amount = BUYER × count`) была неверна — исправлено в 012.

**Субсидия = reconciling bridge net→gross, НЕ доход и НЕ payout.** `net_sales + subsidy_accrual ≈ gross` (bridge замыкается). Субсидия **внутри** revenue (gross), но **вне** `for_pay`. Дедуп: `dedup-subsidies` сворачивает повторные ACCRUAL/DEDUCTION одного `(type, operationType)` до одной записи (first-per-group); `YANDEX_CASHBACK` в gross-мост НЕ входит (уже в цене BUYER).

| Canonical field | Источник API | Transform |
|---|---|---|
| `retail_amount` / `retail_price` | `(BUYER + subsidy) × qty` — **gross** | вычисленное (subsidy = Σ SUBSIDY-ACCRUAL, deduped, split per-item) |
| `net_sales` | `BUYER × qty` — **net** (**НОВОЕ поле**, spec 012) | вычисленное; `price-by-type` даёт line-`:total` → без повторного ×qty |
| `for_pay` | `BUYER − Σ(FEE + AGENCY + DELIVERY_* + PAYMENT_TRANSFER + …, кроме AUCTION_PROMOTION)` — **payout** | **вычисленное** из per-order commissions; субсидия исключена |
| `mp_commission` | `FEE` | агрегат |
| `acquiring_fee` | `PAYMENT_TRANSFER + AGENCY` | агрегат (RFC-10) |
| `delivery_cost` | `DELIVERY_TO_CUSTOMER + EXPRESS_* + CROSSREGIONAL_*` | агрегат |
| `ad_cost` | `AUCTION_PROMOTION` (clearing price) | вне `for_pay` (FR-009) |
| `article` | `item.shopSku` | прямое |
| `operation` | item-level `itemStatus` → order `status` | `DELIVERED→sale`, `RETURNED→return`, `CANCELLED_*→adjustment` |
| `storage_fee`, `acceptance`, `penalty`, `deduction` | — | `nil` (частично покрыты RETURNED_ORDERS_STORAGE / SORTING) |

**Исключение из RFC-15 для YM (spec 012):** на YM **sale-строке** `for_pay` **может быть отрицательным** (`BUYER − commissions < 0`, убыточный SKU — например TS `toTransfer < 0`) — `Math/abs` снят ТОЛЬКО на sale-ветке YM. **Return-строка сохраняет `+abs`** (RFC-15 для пары sale/return; направление даёт `operation_kind`). Инвариант `for_pay ≤ retail_amount` (gross) держится на каждой YM-строке. WB/Ozon остаются `for_pay ≥ 0` без изменений (FR-011 / bit-for-bit).
...
- Возвраты идентифицируются по item-level `itemStatus` (приоритет) → order `status` — приближение.
...
|---|---|---|---|
| `for_pay` | ✅ | ✅ (после B-005) | ✅ (вычисленный; YM sale допускает `< 0`) |
| `retail_amount` (gross) | ✅ | ✅ | ✅ (`(BUYER + subsidy) × qty`) |
| `net_sales` (net) | ⚠️ `nil` (== gross, нет платформенного дисконта) | ⚠️ `nil` (== gross) | ✅ (`BUYER × qty`) |
