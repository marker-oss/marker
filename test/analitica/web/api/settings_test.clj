...
(deftest test-marketplace-returns-verdict-without-saving
  (with-redefs [validate/validate-credentials (fn [_ _] {:valid? true :detail "OK"})
    ...
    (let [resp (settings-api/test-marketplace
                 {:body {:marketplace "wb" :api-token "tok"}})]
      (is (= 200 (:status resp)))
...
(deftest put-marketplace-422-on-invalid
  (with-redefs [validate/validate-credentials (fn [_ _] {:valid? false :detail "bad"})
    ...
    (let [resp (settings-api/put-marketplace
                 {:body {:marketplace "wb" :api-token "bad"}})]
      (is (= 422 (:status resp)))
...
(deftest put-marketplace-saves-and-reloads-on-valid
  (let [saved (atom {})
    ...
    (with-redefs [validate/validate-credentials (fn [_ _] {:valid? true :detail "OK"})
      ...
      (let [resp (settings-api/put-marketplace
                   {:body {:marketplace "wb" :api-token "good"}})]
        (is (= 200 (:status resp)))
  ...
  (let [saved (atom {})]
    (with-redefs [validate/validate-credentials (fn [_ _] {:valid? true :detail "OK"})
      ...
      (settings-api/put-marketplace
        {:body {:marketplace "ozon" :client-id "cid" :api-key "key"}})
      (is (= false (get @saved "mp.ozon.client-id")))
      (is (= true  (get @saved "mp.ozon.api-key"))))))

(deftest unknown-marketplace-400
  (let [resp (settings-api/put-marketplace {:body {:marketplace "foo" :api-token "x"}})]
    (is (= 400 (:status resp)))))
...
(deftest put-marketplace-reload-failure-returns-500-with-saved-true
  ;; I1: reload-config! throws → 500 with :saved? true :reloaded? false
  (with-redefs [validate/validate-credentials (fn [_ _] {:valid? true :detail "OK"})
                settings/set! (fn [& _] nil)
                core/reload-config! (fn [] (throw (RuntimeException. "simulated reload failure")))]
    (let [resp (settings-api/put-marketplace
                 {:body {:marketplace "wb" :api-token "good"}})]
      (is (= 500 (:status resp)))
      (is (true? (get-in resp [:body :saved?])))
      (is (false? (get-in resp [:body :reloaded?]))))))
