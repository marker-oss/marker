...
;; ---------------------------------------------------------------------------
;; specs/002-calculation-audit/verdicts.md is a LIVE operator file and specs/
;; is untracked since the OSS split — on fresh clones (CI) it does not exist.
;; Data tests against it run only where the file is present.
;; ---------------------------------------------------------------------------

(defmacro ^:private deftest-live
  "Like deftest, but the body runs only when the live (gitignored)
   verdicts.md exists; otherwise the test passes vacuously with a note."
  [name & body]
  `(deftest ~name
     (if (.exists (io/file @#'verdict/default-verdicts-path))
       (do ~@body)
       (is true "verdicts.md absent (specs/ untracked) — skipped"))))

;; ---------------------------------------------------------------------------
;; T038 — parser tests
;; ---------------------------------------------------------------------------

(deftest-live parses-all-nine-verdicts
...
(deftest-live extracts-id-and-title-for-each-verdict
...
(deftest-live extracts-conclusion-for-each-verdict
...
(deftest-live extracts-linked-ticket-handling-placeholders
...
(deftest-live parsing-is-idempotent
...
(deftest-live each-verdict-carries-raw-markdown
...
(deftest-live verdict-show-returns-section-for-known-id
...
(deftest-live verdict-show-is-case-insensitive
...
(deftest-live verdict-show-includes-update-postscripts
...
(deftest-live verdict-show-b-009-includes-update-postscript
...
(deftest-live verdicts-list-returns-summary-rows
...
(deftest-live verdicts-list-filters-by-conclusion
